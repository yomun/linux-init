...
# 
# Copyright (C) 2017 - 2020 Jason Mun
# 
...
# 1) 先将 C:\ubuntu 改名为 C:\ubuntu_2004

# 2) 用软件 EasyBCD 2.3　查看现在 ubuntu 20.04 所用的 ID
...
# > bcdedit /set {905b6225-f81c-11e5-8d07-e79e7f21200d} path \ubuntu_2004\winboot\wubildr.mbr

# 4) 将以上制作 C:\winboot　里的 5 个文件, 取代现在原有的 C:\ubuntu_2004\winboot
# 也要将 C:\wubildr 和 C:\wubildr.mbr 更换掉 (是否能进入 Grub 2, 这可是关键..)

# --------------------- Ubuntu 20.04 (进入系统修改设置) ---------------------

# 1) 进入 Grub 2, 选 ubuntu, 按 e, 进入编辑模式
# >> 将 2 处路径 /ubuntu/disks/root.disk 改为 /ubuntu_2004/disks/root.disk
# >> 将 ro quiet splash $vt_handoff 改为 rw single init=/bin/bash
# 然后按 ctrl+x 运行

# 2) 进入 ubuntu 后, 将 /etc/fstab 修改下 root.disk 与 swap.disk 的路径
# sudo vi /etc/fstab
# /ubuntu/ 改为 /ubuntu_2004/
